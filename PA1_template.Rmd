...
```{r stepshistogram, fig.width=5, fig.height=5, results='hide', message=FALSE}
     ...
     p <- p + geom_text(data = NULL, label=paste("Mean = ", mn, sep = ""), x=14000, y=8, hjust = 0)
     p <- p + geom_text(data = NULL, label=paste("Median = ", md, sep = ""), x=14000, y=7, hjust = 0)
     p <- p + xlab(label = "Steps per Day")
     plot(p)
```

With the missing data ignored, the mean of the number of steps per day is `r format(mn, digits = 5)` and the median is `r format(md, digits = 5)`.
...
```{r stepsbyinterval,fig.width=5,fig.height=5, results='hide', message=FALSE}
     ...
     p1 <- p1 + geom_text(data = NULL, label=paste("Max = ", round(mx), sep = ""), x=1500, y=180, hjust = 0)
     p1 <- p1 + geom_text(data = NULL, label=paste("At Interval = ", maxint, sep = ""), x=1500, y=160, hjust = 0)
     p1 <- p1 + theme_bw()
     plot(p1)
```

The interval with the maximum average steps is `r maxint` and the maximum average is `r format(mx, digits=4)`.
...
```{r nareplace, results='hide', message=FALSE, cache=TRUE}
     ...
     for (i in 1:nrow(alldata)){
          ...
          alldata[i,"newsteps"] <- ifelse(is.na(alldata[i,"steps"]), dm, alldata[i,"steps"])
           }
     sumdata3 <- aggregate(newsteps ~ date, data = alldata, sum, na.action = na.omit)
```

The number of rows with missing values is `r numberna`.

```{r newstephistrogram, fig.width=5, fig.height=5, results='hide', message=FALSE}
     ...
     p2 <- p2 + geom_text(data = NULL, label=paste("Mean = ", nmn, sep = ""), x=14000, y=8, hjust = 0)
     p2 <- p2 + geom_text(data = NULL, label=paste("Median = ", nmd, sep = ""), x=14000, y=7, hjust = 0)
     p2 <- p2 + xlab(label = "Steps per Day")
     plot(p2)
```

With the missing data replaced with the interval average, the mean of the number of steps per day is `r format(nmn, digits = 5)` and the median is `r format(nmd, digits = 5)`.  Only the median has shifted slightly.

## Are there differences in activity patterns between weekdays and weekends?

Start by classifying the days as weekend or weekday:

```{r weekdaydata, message=FALSE, results='hide'}
     alldata$days <- weekdays(as.Date(alldata$date, "%Y-%m-%d"))
     alldata$wday <- as.factor(ifelse(alldata$days %in% c("Saturday","Sunday"), "weekend", "weekday"))
...
```{r stepsbyintervalsplit,fig.width=5,fig.height=8, results='hide', message=FALSE}
     require(gridExtra)
     ...
     p3 <- p3 + theme_bw()
     p3 <- p3 + theme(legend.position = "none", axis.text = element_text(size = rel(0.9)), plot.margin = unit(c(3, 3, 1, 1), "lines"))
     plot(p3)
```
